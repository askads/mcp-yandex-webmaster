...
- **Сервер больше не завершается из-за конфигурации.** При отсутствии `YANDEX_OAUTH_TOKEN`
  он стартует, отвечает на `initialize` и `tools/list`, а в `instructions` предупреждает
  модель, что оператор должен задать переменную окружения и перезапустить сервер. Прежний
  текст стартовой ошибки («YANDEX_OAUTH_TOKEN is required…») сохранён дословно и
  возвращается теперь при вызове инструмента (`CredentialsError`, ответ с `isError`) — до
  ретраев, автоопределения `user_id` и обращения к сети. Невалидный `YANDEX_USER_ID`
