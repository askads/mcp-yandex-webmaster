...
- **16 инструментов** для сайтов, диагностики, поисковых запросов, индексации, sitemap и внешних ссылок.
...
Нужны Node.js 20+ и OAuth-токен Яндекса с доступом к Вебмастеру. `npx` скачает сервер при первом запуске — отдельно устанавливать пакет не нужно.

1. [Получите токен](#доступ-к-данным) и добавьте сервер в своё AI-приложение. Выберите инструкцию ниже.
2. Откройте новый диалог и напишите: **«Какие критичные проблемы сейчас видит диагностика на моём сайте?»**
...
1. Создайте приложение на [oauth.yandex.ru](https://oauth.yandex.ru) и в правах доступа выберите **API Яндекс Вебмастера**.
2. Получите OAuth-токен по [инструкции Яндекс OAuth](https://yandex.ru/dev/id/doc/ru/access).
3. Передайте токен в переменной `YANDEX_OAUTH_TOKEN` при подключении сервера.
